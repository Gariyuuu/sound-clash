...
- **Repository type:** Single Next.js App Router application. It lives at `~/Projects/sound-clash/` and **has its own independent git repository** (`~/Projects/sound-clash/.git`, not the parent `~/Projects` tree), pushed to `https://github.com/Gariyuuu/sound-clash.git` (remote `origin`, branch `main`). This is a correction of earlier-session claims that no git repo existed anywhere for this project — that was true through the sixth session but is no longer accurate; see `PROJECT_STATE.md`'s "Git state" for the verified current state.
...
- **Documentation gap (found during a 2026-08-07 checkpoint pass, not yet fully written up):** `git log` shows five real, committed changes — the sixth-session infra-provisioning work described above, then a seventh session (sign-in routing fix, sign-out access on every page, theme wheel, Play-vs-AI mode — this one IS covered in `SESSION_LOG.md`/`TASKS.md`), and then **four further commits with no `SESSION_LOG.md`/`FEATURES.md` write-up at all**: a Career Mode entry point (`src/app/career/`, `src/lib/game/career.ts`) with 10-genre filtering, a YouTube answer-leak fix (video title/channel overlay was visible during gameplay), background presets expanded 14→20 (added Aurora/Desert/Midnight City/Sakura/Volcano/Frost + a grain overlay), AI-opponent timing softened and the buzz timer's default/max raised (10s→20s / 20s→30s), and a scoreboard live-update bug fix + visual redesign. See `PROJECT_STATE.md`'s "Documentation gap" section for the exact commit hashes/messages. Code-level verification (`tsc`/`lint`/`build`) covers this code since it's already committed and part of the tree, but feature-level docs (`FEATURES.md`, `UI_SYSTEM.md`, `API_REFERENCE.md`) have **not** been updated for it yet — treat any doc claim about background count, buzz timer defaults, or the absence of a "Career Mode" as unreliable until that catch-up happens.
...
src/
  ...
  lib/
    ...
    theme-backgrounds.ts               # CSS gradient definitions for the 20 named background presets (expanded from 14; see PROJECT_STATE.md "Documentation gap")
