...
- **The light theme is not designed** (found 2026-09-05, see `UI_SYSTEM.md`'s "Known defect"). `/settings` offers Light and defaults to System, but `BackgroundLayer` paints a dark preset gradient behind all content in *both* schemes, so a light-OS player reads near-black text on a near-black ground — verified unreadable on `/room/[code]`'s form labels. Fix is a light-theme design pass: light variants for `.glass`/`.glass-strong`/`.neon-*` plus the 46 hardcoded `white/<alpha>` utilities across 15 files moved to a token. Deliberately out of scope for the W4 overhaul that found it. Interim alternative if a design pass isn't wanted: drop Light from the `/settings` selector and force `dark`, which is what the app is actually designed for.
