...
- **Custom named backgrounds (Cyberpunk, Galaxy, Neon, etc.):** modeled as a 15-value union type in `settings-store.ts` (`ThemeBackground`). **Now implemented (session four):** `src/lib/theme-backgrounds.ts` maps each of the 14 presets to a CSS gradient string (pure-CSS approximations, no bundled images), and `src/components/providers/background-layer.tsx` (mounted in the root layout) renders the chosen one as a fixed full-viewport layer behind all content. `/settings` provides a swatch-grid picker plus a custom-image upload (stored as a `localStorage` data URL — see `DECISIONS.md` D-012 for why not real file/blob storage).
